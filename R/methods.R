...
#' @examples
#' sub_object = pbmc
#' LR = read.delim("C:/Users/YWJ/Desktop/库/Ramilowski2015/Ramilowski_human.txt", header=TRUE)
#' sender_celltypes = c("CD8 T","NK")
#' receiver_celltypes = c("DC","Platelet")
#' interact_combination(sub_object,LR,marker_filter=FALSE,avg_log2FC=0.5,padj=0.05,sender_celltypes,receiver_celltypes)
#'
#' @export
#'
interact_combination <- function(sub_object,LR,marker_filter=FALSE,avg_log2FC=1,padj=0.05,sender_celltypes,receiver_celltypes){
  ...
  # mean_dc <- mean_lr[unique(c(interactions_combination$ligand,interactions_combination$receptor)),]
  # mean_dc$gene <- rownames(mean_dc)
  # mean_dc <- melt(mean_dc)
  # mean_l <- merge(interactions_combination,mean_dc,by.x="ligand",by.y="gene")
  # mean_l <- mean_l[mean_l$from == mean_l$variable,]
  # mean_r <- merge(mean_l,mean_dc,by.x="receptor",by.y="gene")
  # mean_lr <- mean_r[mean_r$to == mean_r$variable.y,]
  # interactions_combination <- mean_lr[,c("ligand","receptor","from","to","value.x","value.y","avg_log2FC.l","avg_log2FC.r")]
  # colnames(interactions_combination) <- c("ligand","receptor","from","to","mean_ligand","mean_receptor","avg_log2FC.l","avg_log2FC.r")
...
#' @examples
#' hnscc_primary <- readRDS("C:/Users/YWJ/Documents/hnscc_primary.rds")
#' lrDB <- CellChatDB.human
#' receiver_celltypes=c("T cell","CAF","Endothelial","Macrophage","myofibroblast")
#' sender_celltypes="primary"
#' interaction1 <- find_interactions(object=hnscc_primary,DB=lrDB,celltype="celltype",method="expression_threshold",
#'                                   receiver_celltypes=receiver_celltypes,
#'                                   sender_celltypes=sender_celltypes,threshold=3)
#' interaction2 <- find_interactions(object=hnscc_primary,DB=lrDB,celltype="celltype",method="expression_product",
#'                                   receiver_celltypes=receiver_celltypes,
#'                                   sender_celltypes=sender_celltypes,product_top=0.5)
#' interaction3 <- find_interactions(object=hnscc_primary,DB=lrDB,celltype="celltype",method="differential_combinations",
#'                                   receiver_celltypes=receiver_celltypes,
